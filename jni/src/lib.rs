...
pub extern "system" fn Java_xyz_bluspring_unitytranslate_library_UnityTranslateLib_createInstance<'local>(
    ...
    let tokenizer_model_value = tokenizer_model_path.to_string();
    ...
    let tokenizer: UnityTranslateTokenizer = match (tokenizer_type_value) {
        ...
        TokenizerType::Bpe => {
            let bpe_model_data = fs::read_to_string(tokenizer_model_value).expect("Couldn't read BPE model file!");
    ...
    if let Ok(translator) = translator_result {
        Box::into_raw(Box::new(translator)) as jlong
