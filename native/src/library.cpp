...
extern "C" {
    ...
    __export void batchTranslate(const UnityTranslateLibInstance* instance, const int arrayLength, const char** textToTranslate, const char** results) {
        vector<string> textToTranslateVec(arrayLength);
        vector<string> resultsVec(arrayLength);

        for (int i = 0; i < arrayLength; i++){
            textToTranslateVec[i] = textToTranslate[i];
        }

        batchTranslate(instance, textToTranslateVec, resultsVec);

        for (int i = 0; i < arrayLength; i++) {
            string result = resultsVec[i];
    ...
    JNIEXPORT jlong JNICALL Java_xyz_bluspring_unitytranslate_library_UnityTranslateLib_createInstance(JNIEnv* env, jobject thisObj, jstring toLang, jstring translatorModelPath, jint type, jstring tokenizerModelPath, jboolean useCuda) {
        const char* toLangNative = env->GetStringUTFChars(toLang, nullptr);
        const char* translatorPathNative = env->GetStringUTFChars(translatorModelPath, nullptr);
        const char* tokenizerPathNative = env->GetStringUTFChars(tokenizerModelPath, nullptr);

        const auto tokenizerType = static_cast<TokenizerType>(type);
        UnityTranslateLibInstance* instancePtr = createInstance(toLangNative, translatorPathNative, tokenizerType, tokenizerPathNative, useCuda);

        // free time
        env->ReleaseStringUTFChars(toLang, toLangNative);
        env->ReleaseStringUTFChars(translatorModelPath, translatorPathNative);
        env->ReleaseStringUTFChars(tokenizerModelPath, tokenizerPathNative);
        
        return reinterpret_cast<jlong>(instancePtr);
    }
    ...
    JNIEXPORT void JNICALL Java_xyz_bluspring_unitytranslate_library_UnityTranslateLib_batchTranslate(JNIEnv* env, jobject thisObj, jlong instance, jobjectArray textToTranslate, jobjectArray results) {
        const int arrayLength = env->GetArrayLength(textToTranslate);
        vector<string> textToTranslateVec(arrayLength);
        const vector<string> resultsVec(arrayLength);

        for (int i = 0; i < arrayLength; i++) {
            const jobject textObj = env->GetObjectArrayElement(textToTranslate, i);
            const auto jstr = static_cast<jstring>(textObj);
            const char* textCharArray = env->GetStringUTFChars(jstr, nullptr);

            if (textCharArray == nullptr){
                textToTranslateVec[i] = "(unknown)";
                continue;
            }

            // copy string, so we can free it for Java.
            const int length = env->GetStringLength(jstr);
            const auto textStr = string(textCharArray, length);
            textToTranslateVec[i] = textStr;

            // free for Java's sake
            env->ReleaseStringUTFChars(jstr, textCharArray);
            env->DeleteLocalRef(textObj);
        }

        const auto instancePtr = reinterpret_cast<UnityTranslateLibInstance*>(instance);
        batchTranslate(instancePtr, textToTranslateVec, resultsVec);

        // now to write to the results array
        for (int i = 0; i < arrayLength; i++){
            const jstring textJni = env->NewStringUTF(resultsVec[i].c_str());
            env->SetObjectArrayElement(results, i, textJni);
        }
    }

    JNIEXPORT void JNICALL Java_xyz_bluspring_unitytranslate_library_UnityTranslateLib_freeInstance(JNIEnv* env, jobject thisObj, const jlong instance) {
        const auto instancePtr = reinterpret_cast<UnityTranslateLibInstance*>(instance);
...
__export void batchTranslate(const UnityTranslateLibInstance* instance, const vector<string>& textToTranslate, vector<string> results) {
    const int arrayLength = textToTranslate.size();
    vector<vector<string>> tokenizedTexts;

    for (int i = 0; i < arrayLength; i++) {
        string text = textToTranslate[i];
    ...
    for (int i = 0; i < arrayLength; i++) {
        ...
        const string parsed = instance->tokenizer->decode(tokens);
        results[i] = parsed;
    }
}